...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(config.epochs), miniters=1, unit="epoch"):
        train_loss, valid_loss, valid_metrics, y_p, y, train_r2, train_mae, train_mse = trainer.train(epoch)
    ...
    print("Train metrics: MAE: %.4f, MSE: %.4f, R²: %.4f" % (best_Tmae, best_Tmse, best_Tr2))
    ...
    test_metrics, y_true, y_pred, area_ha = trainer.test(test_data_loader)
    ...
    df.to_csv("/mnt/c3691c07-b3dd-4b44-a41f-1a86553ea058/luis/the_best_ones/the_new_one/test_all_mse_dp03.csv")
